...
class YOLODet3dDataset(BaseDataset):
    """
    Dataset class for loading object detection and/or segmentation labels in YOLO format.
    ...
    def cache_labels(self, path=Path("./labels.cache")):
        ...
        with ThreadPool(NUM_THREADS) as pool:
            ...
            pbar = TQDM(results, desc=desc, total=total)
            for im_file, lb_2d, shape, lb_3d, nm_f, nf_f, ne_f, nc_f, msg in pbar:
                nm += nm_f
                nf += nf_f
                ne += ne_f
                nc += nc_f
                    ...
                    x["labels"].append(
                        {
                            ...
                            "cls": lb_2d[:, 0:1],  # n, 1
                            "bboxes": lb_2d[:, 1:],  # n, 4
                            "xyz_3d": lb_3d[:, 0:3],  # n, 3
                            "whl_3d": lb_3d[:, 3:6],  # n, 3
                            "yaw_3d": lb_3d[:, 6:7],  # n, 1
                            "normalized": True,
                            "bbox_format": "xywh",
                        }
                    )
    ...
    def collate_fn(batch):
        ...
        for i in range(len(new_batch["batch_idx"])):
            new_batch["batch_idx"][i] += i  # add target image index for build_targets()
        new_batch["batch_idx"] = torch.cat(new_batch["batch_idx"], 0)
        # print(f"\ncollate: {new_batch['bboxes'].shape}, {new_batch['xyz_3d'].shape}\n")
        return new_batch
